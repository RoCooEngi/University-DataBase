...
# global session counter during one url reqest
session_counter = 0

# Global stop flag
stop_flag = mp.Value('b', False)  # shared boolean across processes
...
def subject_multi_process(programs: list, USERNAME:str, PASSWORD: str):
    '''Makes multiple parsing of subjects'''
    session = create_session(USERNAME, PASSWORD)
...
if __name__ == '__main__':
    ...
    #     log_request_error('students')
    # else:
    #     cursor.execute('SELECT id, name FROM programs')
    #     row_prog = cursor.fetchall()
    #     pprint(row_prog)